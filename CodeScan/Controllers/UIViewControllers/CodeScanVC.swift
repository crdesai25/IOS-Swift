...
class CodeScanVC: UIViewController, AVCaptureMetadataOutputObjectsDelegate, SelectedTypesDelegate, UIGestureRecognizerDelegate {
        ...
        if status == .authorized {
            ...
            AVCaptureDevice.requestAccess(for: .video, completionHandler: { granted in
                if granted {
                    ...
                    self.tapToDismiss = UITapGestureRecognizer(target: self, action: #selector(self.dismissAlert))
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if (captureSession?.isRunning == false) {
            captureSession.startRunning() // Start Camera Scanning
        }
    }
